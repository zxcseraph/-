...
fiorun()
{
	...
	BWnum=`grep BW ${testnamet}|tail -1|awk -F':' '{print $2}'|awk -F',' '{print $1}'|awk -F'=' '{print $2}'|awk '{if(substr($0,length())=="k"){ print substr($0,0,length()-1)*1000}else{print $0}}'`
...
rwgo()
{
	...
	for rwi in ${rw[*]};do
		finalfilename=${name}_${rwi}_final
		rwforrwmix=$rwi
		setrwmix;
		...
		echo "fio -filename=${lvname} -direct=1 -iodepth 1 -thread -rw=${rwi} ${rwmicommand} -ioengine=${rwiokoengine} -bs=${rwiokbs} -size=${finalsize} -numjobs=${rwiokjob} -runtime=${finalruntime} -group_reporting -name=${finalfilename}" >>$finalcommandfile
	done
}

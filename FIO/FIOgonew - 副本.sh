...
lvname=/dev/vg3parfc/fctestlv				#要改
name=mytest
...
njobrun()
{
	...
	do
		...
		log4s debug "njobrun: 当前获取到的num1-2分别为：$num1 $num2"
		log4s debug "njobrun: 当前njobs1-2分别为：$njob1 $njob2"
		...
		then
			...
			log4s debug "njobrun: 2号任务性能大于1号的百分之$chajubili且为正，在上升通道中，新的1号任务njobs为$njob1，新的2号任务为$njob2"
			njob1=$njob2
			let njob2=njob2+1
			num1=$num2
			num2=$(fiorun $filename $bst $sizet $ioenginet $rwt $njob2 $rwmicommandt)
		...
		then
			...
			log4s debug "njobrun: 1号任务性能大于2号的百分之$chajubili且为负，在下降同道中，新的1号任务njobs为$njob1，新的2号任务为$njob2"
			njob1=$njob2
			let njob2=njob2+1
			num1=$num2
			num2=$(fiorun $filename $bst $sizet $ioenginet $rwt $njob2 $rwmicommandt)
			...
			log4s debug "njobrun: 当前增大或者减少少于百分之$chajubili，认为进入平滑曲线，当前1号任务njobs为$njob1，其bw值为$num1，当前2号任务njobs为$njob2，其bw值为$num2，当前在平滑区次数为$njobtemp "
			if [ $njobtemp = 5 ]
			then
				log4s debug "在平滑区已经保持5次，退出当前任务"
...
fenxi()
{
	...
	then
		...
		then
			...
			fx_r_slat_stdev=`awk -v t="$fx_r_slat_avg" 'BEGIN {printf "%d",t*1000}'`
		...
		then
			...
			fx_r_slat_stdev=`awk -v t="$fx_r_slat_avg" 'BEGIN {printf "%.5f",t/1000}'`
		fi
		...
		then
			...
			fx_r_clat_stdev=`awk -v t="$fx_r_clat_avg" 'BEGIN {printf "%d",t*1000}'`
		...
		then
			...
			fx_r_clat_stdev=`awk -v t="$fx_r_clat_avg" 'BEGIN {printf "%.5f",t/1000}'`
		fi
		...
		then
			...
			fx_r_lat_stdev=`awk -v t="$fx_r_lat_avg" 'BEGIN {printf "%d",t*1000}'`
		...
		then
			...
			fx_r_lat_stdev=`awk -v t="$fx_r_lat_avg" 'BEGIN {printf "%.5f",t/1000}'`
		fi
		let yushu=hanghao%2
		if [ X$yushu = X1 ]
		then
	...
	then
		...
		then
			...
			fx_w_slat_stdev=`awk -v t="$fx_w_slat_avg" 'BEGIN {printf "%d",t*1000}'`
		...
		then
			...
			fx_w_slat_stdev=`awk -v t="$fx_w_slat_avg" 'BEGIN {printf "%.5f",t/1000}'`
		fi
		...
		then
			...
			fx_w_clat_stdev=`awk -v t="$fx_w_clat_avg" 'BEGIN {printf "%d",t*1000}'`
		...
		then
			...
			fx_w_clat_stdev=`awk -v t="$fx_w_clat_avg" 'BEGIN {printf "%.5f",t/1000}'`
		fi
		...
		then
			...
			fx_w_lat_stdev=`awk -v t="$fx_w_lat_avg" 'BEGIN {printf "%d",t*1000}'`
		...
		then
			...
			fx_w_lat_stdev=`awk -v t="$fx_w_lat_avg" 'BEGIN {printf "%.5f",t/1000}'`
		fi
		let yushu=hanghao%2
		if [ X$yushu = X1 ]
		then
...
gohtml()
{
	...
	xhtml "<th rowspan="2">rw</th>"
	xhtml "<th rowspan="2">测试方式</th>"
	xhtml "<th rowspan="2">bs大小</th>"
	xhtml "<th rowspan="2">jobs数量</th>"
	xhtml "<th rowspan="2">IOPS</th>"
	xhtml "<th rowspan="2">BW</th>"
	xhtml "<th colspan="5">slat</th>"
	xhtml "<th colspan="5">clat</th>"
	xhtml "<th colspan="5">lat</th>"
	xhtml "</tr>"

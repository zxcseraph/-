...
InputAndCheck()
{
	...
	do
		...
		then
			read -p "请输入本机数据库对外提供服务的ip："				serverip
			read -p "请输入本机数据库对外提供服务的端口："			serverport
			read -p "请设置需要安装的客户端个数，一次最多5个："	clientcount
			if [ Xclientcount != X1 ] && [ Xclientcount != X2 ] && [ Xclientcount != X3 ] && [ Xclientcount != X4 ] && [ Xclientcount != X5 ]
			then

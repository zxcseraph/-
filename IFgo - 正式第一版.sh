...
jiaobenming=IFgo.sh					#如果脚本要改名，这里一定要改
...
CheckP()
{
	...
	if [ X$XTBANBEN != X5.9 ] && [ X$XTBANBEN != X6.0 ] && [ X$XTBANBEN != X6.1 ] && [ X$XTBANBEN != X6.2 ] && [ X$XTBANBEN != X6.3 ] && [ X$XTBANBEN != X6.4 ] && [ X$XTBANBEN != X6.5 ] && [ X$XTBANBEN != X6.6 ] && [ X$XTBANBEN != X6.7 ] && [ X$XTBANBEN != X6.8 ] && [ X$XTBANBEN != X6.9 ]
	then
...
InputAndCheck()
{
	while [[ X$hdrflag != Xhdr  && X$hdrflag != Xonly && X$hdrflag != Xsec && X$hdrflag != Xpri ]]
	do
		read -p "请输入hdr标识，如果是双机主备库写hdr（需要在主机执行该脚本），单机写only： " hdrflaginput
	...
	do
		...
		then
			log4s info "hdr配置开始"
			read -p "请输入ssh端口号："  sshport
			echo "下面开始输入数据库实例名，也就是在sqlhosts中配置的数据库实例名"
			
			#priINFORMIXSERVER参数
			if [ $peizhiqueren = XXXXXX ]
			then
			...
			if [ $peizhiqueren = XXXXXX ]
			then
				read -p "请输入主机业务实例实例名，默认为appdb1 ： " tpriDBSERVERALIASES
			fi
			if [ $peizhiqueren != XXXXXX ]
			then
				read -p "请输入主机业务实例实例名，刚才输入的是$priDBSERVERALIASES ： " tpriDBSERVERALIASES
			fi
			...
			if [ $peizhiqueren = XXXXXX ]
			then
				read -p "请输入主机心跳线ip："	priip
				read -p "请输入备机心跳线ip："	secip
				read -p "请输入主机业务ip："		priappip
				read -p "请输入备机业务ip："		secappip
			fi
			if [ $peizhiqueren != XXXXXX ]
			then
			...
			log4s debug "请输入主机心跳线ip： $priip"
			log4s debug "请输入备机心跳线ip： $secip"
			log4s debug "请输入主机业务ip：   $priappip"
			log4s debug "请输入备机业务ip：   $secappip"
	...
	do
		read -p "是否需要创建PV，如果创建pv，请输入y/n：" shifouchuangjianpv;
	done
	if [ $shifouchuangjianpv = y ] || [ $shifouchuangjianpv = Y ]
	then
		...
		then
			...
			do
				...
				if [ X$shifouqueredevname = XXXXXXX ]
				then
	...
	then
		...
		do
			if [ $devname = XXXXXX ]
			then
				read -p "请输入pv的名称，比如/dev/sdb："  devname
				log4s info "pv是手动创建，所以需要输入创建的pv名称，为：$devname"
			fi
			read -p "请输入vg的名称，比如dbvg：" vgname;
			read -p "请务必确认硬盘路径是否为$devname，如果输入错误将造成不可预知的问题[Y/N]：" shifouqueredevname1
			if [ X$shifouqueredevname1 = XXXXXXX ]
			then
	...
	then
		log4s info "需要创建lv"
		if [ $shifouchuangjianvg = N ] || [ $shifouchuangjianvg = n ]
		then
		...
		do
			...
			read -p "是否需要调整lv大小，请输入y/n：" shifoutiaozhenglvsize;
		done
		if [ $shifoutiaozhenglvsize = y ] || [ $shifoutiaozhenglvsize = Y ]
		then
			...
			do
				...
				if [ X$shifouquerenlvsize = XXXXXXX ]
				then
					shifouquerenlvsize=n
				fi
	...
	then
		...
		do
			...
			then
				...
				echo "如果没有这个dbs，请输入0或者N或n或nul或者不输入都可以"

...
/*begin: chengg add for c++ slp 20090921*/
void TiXmlElement::StreamOutReadability ( TIXML_OSTREAM * out, int dept) const
{
    int i;
    for ( i = 0; i < dept; i++ )
    {
    ...
    {
        (*out) << " ";
        attrib->StreamOutReadability( out, dept );
    }
    ...
    {
        (*out) << ">" ;
        firstChild->StreamOutReadability( out, dept + 1 );
        (*out) << "</" <<value << ">" ;
    }
    else
    {
        ...
        for ( node = firstChild; node; node = node->NextSibling() )
        {
            ...
            node->StreamOutReadability( out, dept + 1 );
        }
        (*out) << "\n" ;
        for ( i = 0; i < dept; ++i )
            (*out) << "    " ;
...
/*end: chengg add for c++ slp 20090921*/
...
/*begin: chengg add for c++ slp 20090921*/
void TiXmlDocument::StreamOutReadability ( TIXML_OSTREAM * out, int dept) const
{
    TiXmlNode * node;
    for ( node = FirstChild(); node; node = node->NextSibling() )
    {
        node->StreamOutReadability( out, dept );
        (*out) << "\n" ;
...
/*begin: chengg add for c++ slp 20090921*/
void TiXmlAttribute::StreamOutReadability ( TIXML_OSTREAM * out, int dept) const
{
    StreamOut(out);
}
/*end: chengg add for c++ slp 20090921*/
...
/*begin: chengg add for c++ slp 20090921*/
void TiXmlComment::StreamOutReadability ( TIXML_OSTREAM * out, int dept) const
{
...
/*begin: chengg add for c++ slp 20090921*/
void TiXmlText::StreamOutReadability ( TIXML_OSTREAM * out, int dept) const
{
    StreamOut(out);
}
/*end: chengg add for c++ slp 20090921*/
...
/*begin: chengg add for c++ slp 20090921*/
void TiXmlDeclaration::StreamOutReadability ( TIXML_OSTREAM * out, int dept) const
{
    StreamOut(out);
}
/*end: chengg add for c++ slp 20090921*/
...
/*begin: chengg add for c++ slp 20090921*/
void TiXmlUnknown::StreamOutReadability ( TIXML_OSTREAM * out, int dept) const
{
    for ( int i = 0; i < dept; i++ )
        (*out) << "    " ;
    StreamOut(out);
...
/*add by yang@iip for common use*/
TiXmlDocument * loadXmlCfgFile( bool exitOnError, char *xmlFileName )
{
    if ( !xmlFileName )
    {
        printf( "Unknown config XML file name!\n" );
        return NULL;
    }
    //printf("xml filename: %s\n",xmlFileName);
    TiXmlDocument *pXmlTree = new TiXmlDocument( xmlFileName );
    if ( !pXmlTree )
    {
        printf( "(%s:%d)Alloc XML treee for %s error !", __FILE__, __LINE__, xmlFileName );
        if ( exitOnError )
        {
            exit( 0 );
        }
    ...
    if ( !pXmlTree->LoadFile() )
    {
        printf( "Load config file %s failed, %s", xmlFileName, pXmlTree->ErrorDesc() );
        delete pXmlTree;
        if ( exitOnError )
        {
            exit( 0 );

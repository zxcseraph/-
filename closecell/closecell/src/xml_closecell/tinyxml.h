...
/* redefine assert macro to avoid abnormal exit. by zhoujunfeng */
#ifdef assert
#undef assert
#endif
#define assert(ex) ((ex) ? (void)0 : throw 0)
...
protected:
	...
	virtual void StreamOutReadability ( TIXML_OSTREAM * out, int dept) const = 0; /*chengg add for c++ slp 20090921*/
	...
	*/
	...
	/* add errorLine to provide more error information. by zhoujunfeng */
...
public:
	...
	*/
	...
	virtual void StreamOutReadability ( TIXML_OSTREAM * out, int dept) const; /*chengg add for c++ slp 20090921*/
	// [internal use]
...
#ifdef TIXML_USE_STL
	/// Constructor.
	TiXmlDeclaration(const std::string& _version, const std::string& _encoding, const std::string& _standalone) : TiXmlNode(TiXmlNode::DECLARATION)
	{
		version = _version;
		encoding = _encoding;
...
public:
...
#ifdef TIXML_USE_STL
	/// Constructor.
	TiXmlDocument(const std::string& documentName) : TiXmlNode(TiXmlNode::DOCUMENT)
	{
		value = documentName;
		error = false;
	}
	...
	*/
	...
	void ClearError()
	{
		error = false; errorId = 0; errorDesc = "";
	}
	...
	void SetError(int err)
	{
		...
		/* add errorLine to provide more error information. by zhoujunfeng */
		char chErrorLine[24];
		sprintf(chErrorLine, "Line %d: ", errorLine);
		errorDesc = chErrorLine;
		errorDesc += errorString[errorId];
	}

	/*begin: chenggang modify from protected to public for c++ slp 20090919*/
	virtual void StreamOut(TIXML_OSTREAM* out) const;
	virtual void StreamOutReadability ( TIXML_OSTREAM * out, int dept=0) const;
	virtual TiXmlNode* Clone() const;
	/*end: chenggang modify from protected to public for c++ slp 20090919*/
...
TiXmlDocument * loadXmlCfgFile( bool exitOnError, char *xmlFileName); //added by wangxb 20070509

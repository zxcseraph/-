drop procedure add_par_for_int;
create procedure add_par_for_int(p_tabname varchar(50))
        ...
        define v_highvalue int;
		define v_partition_info varchar(50);
		define v_maxvalue int;
        define i,j int;
        define v_endvalue int;
        define v_partition,v_addpartname varchar(20);
        define sql varchar(255);
		define v_procname varchar(50);
        set debug file to '/tmp/add_par_for_int.dbg';
        trace on;
		let v_procname ='add_par_for_int';
		for j in (1 to 4 step 1)
				...
				select max(endtime::int) into v_highvalue from fragtabinfo where tabname=p_tabname;
				select partition into v_partition_info from fragtabinfo where tabname=p_tabname and endtime=v_highvalue;
				if v_highvalue > (v_maxvalue + v_time_granularity * 4) ::int then 
					  exit for;
				end if;
				let v_endvalue= ( v_highvalue + v_time_granularity ) ::int;
				let v_partition=(trim(substr(v_partition_info,2,20)) + 1)::int; 
				let v_addpartname='p'||v_partition;
				...
				for i in (1 to 100 step 1)        
				  ...
				  then
					  let sql='alter fragment on table '||p_tabname||' add partition '||v_addpartname||'('||v_column_fragment||'>='''||v_highvalue||''' and '||v_column_fragment||'<'''||v_endvalue||''' )in '||v_dbs_name;
...
--execute procedure add_par_for_int('txcy_deliverinfo');
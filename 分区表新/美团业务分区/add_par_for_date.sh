...
addfrag()
{
date >>$dodir/add_par_for_date.log 
for i in {1..5} 
do
(time dbaccess $1 <<!
 execute procedure add_par_for_date();
!
)  1>> $dodir/add_par_for_date.log 2>&1
echo $i
echo $i >> $dodir/add_par_for_date.log 
mv $dodir/add_par_for_date.dbg $dodir/add_par_for_date.dbg.$dt.$i
done
date >>$dodir/add_par_for_date.log
}
if [ X$1 = X ]
...
then
	...
	if [ $errornum != 0 ]
	then
		log4s error "数据库 $1 中，存 $errornum 个在错误，所以不进行添加分片操作，具体错误请查看alarm.log"
	else
		log4s info "数据库 $1 中，一切正常，开始添加分片"
		addfrag $1
	fi
fi
if [ X$2 = Xmanual ]

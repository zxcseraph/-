...
fengeHour=0
fengeMin=4
dodir=/home/icaa/zxcbin/    #操作目录，也就是脚本所放的目录
bakdir=$dodir/bak
rizhilog=/webjoin/ebupt/logs/soap_prbt_tj3/webjoin.log  #日志全路径
log=$dodir/root.log
...
fengetag=".`date -d "yesterday" +%Y-%m-%d`"   #分割后缀名。待扩展
RingString="UserToneManage.orderTone"
RingString1="toneType=1"
RingsetString="UserToneManage.orderTone"
RingsetString1="toneType=2"
AddToneString="UserToneManage.addSetting"
UserManagerString="UserManage.subscribe"
...
tongji()
{
	...
	then
		...
		#第一次判断是否为负，如果为负则加上前一天的日志重新来一次
		if [ $chaallnum -lt 0 ] || [ $chaoknum -lt 0 ]
		then
			log4s "$dS $4 差值计算为负，进入跨天计算模式"
			#更新差值重新统计
			grep "$GrepString" $lasttimefile|grep "$5" > $logtempname
			grep "$GrepString" $3|grep "$5" >> $logtempname
			#当前获取的总数
			newallnum=`wc -l $logtempname|awk '{print $1}'`
			#当前获取的成功总数
			newoknum=0
			for i in ${filter[*]}; do
		    newoktempnum=`grep "$i" $logtempname|wc -l|awk '{print $1}'`
		    let newoknum=newoknum+newoktempnum
		  done
		  #更新all值重新统计cha值
		  let chaallnum=newallnum-oldallnum
			let chaoknum=newoknum-oldoknum
		fi
		...
		if [ $chaallnum -lt 0 ]
		then
			log4s "$dS $4 的总数差值为负数 $chaallnum ，自动修正为0"
			chaallnum=0
		fi
		if [ $chaoknum -lt 0 ]
		then
			log4s "$dS $4 的正确差值为负数 $chaoknum ，自动修正为0"
			chaoknum=0
		fi
...
shangchuan()
{
	#第一个参数是读取文件
	catlog=$1
	subuserall=`grep usersubscribe $catlog|tail -1|grep $dS|awk -F'|' '{print $5}'`
	subuserok=`grep usersubscribe $catlog|tail -1|grep $dS|awk -F'|' '{print $6}'`
	addsetall=`grep addSetting $catlog|tail -1|grep $dS|awk -F'|' '{print $5}'`
	addsetok=`grep addSetting $catlog|tail -1|grep $dS|awk -F'|' '{print $6}'`
	ringsetall=`grep ringsetorder $catlog|tail -1|grep $dS|awk -F'|' '{print $5}'`
	ringsetok=`grep ringsetorder $catlog|tail -1|grep $dS|awk -F'|' '{print $6}'`
	ringall=`grep ringorder $catlog|tail -1|grep $dS|awk -F'|' '{print $5}'`
	ringok=`grep ringorder $catlog|tail -1|grep $dS|awk -F'|' '{print $6}'`
	jiaoyanflag=`grep usersubscribe $catlog|tail -1|grep $dS|awk -F'|' '{print $1}'`
	...
	echo "insert into inms_pm_data (jobid,host,account,endtime,seqno,inserttime,statnum,stat01,stat02,stat03,stat04,stat05,stat06,stat07,stat08) values (\"031017\",\"$hostname\",\"$wai\",\"$dSn\",1,\"$dSn\",8,\"$subuserall\",\"$subuserok\",\"$addsetall\",\"$addsetok\",\"$ringsetall\",\"$ringsetok\",\"$ringall\",\"$ringok\")"|dbaccess ${servername}@${databasename}
	if [ $? = 0 ]
	then
	...
	echo "insert into inms_pm_datamgr (jobid,host,account,endtime,datanum) values (\"031017\",\"$hostname\",\"$wai\",\"$dSn\",1)"|dbaccess ${servername}@${databasename}
	if [ $? = 0 ]
	then

...
dodir=/tmp/zxc         #脚本工作目录，所有操作均会在这个目录执行
...
servername=cdssip                       #数据库servername
databasename=cdssip                     #数据库的实例名   比如newip@dbm1，newip就是实例名，dbm1就是servername
...
d1dayago=`date -d "-5 days" +"%Y%m%d"`
d2dayago=`date -d "-6 days" +"%Y%m%d"`
#音频相关
#音频彩铃呼叫话单文件命名规则
...
crbtorifilenames=("CN41" "CN42" "CN51" "CN52" "AS61" "AS62")
#音频文件存放目录
crbtfiledir=/home/icaa/loudan/tjziyuan/crbt
#音频原始文件存放目录
crbtfileoridir=/home/icaa/loudan/tjziyuan/crbtyuanshi
#音频文件临时工作目录
...
#视频原始文件主机名，同音频
declare -A truevrbthostname=(["AS61"]="TJ-as61" ["AS62"]="TJ-as62")
vrbtorifilenames=("AS61" "AS62")
#视频文件存放目录
vrbtfiledir=/tmp/zxc/20180921/finall
#视频原始文件存放目录
vrbtfileoridir=/tmp/zxc/20180921/orilog
#视频文件临时工作目录

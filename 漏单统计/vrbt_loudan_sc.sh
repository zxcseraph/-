#!/bin/sh
dodir=/webjoin/video/loudan/workdir         	#脚本工作目录，所有操作均会在这个目录执行
...
servername=cdssdb2                      #数据库servername
databasename=cdss                       #数据库的实例名   比如newip@dbm1，newip就是实例名，dbm1就是servername
...
#declare -A truecrbthostname=(["AS201"]="as201" ["AS202"]="as202" ["AS221"]="as221" ["CAS031"]="cas031" ["CAS032"]="cas032" ["CAS033"]="cas033" ["CAS041"]="cas041" ["CAS042"]="cas042" ["CAS051"]="cas051" ["CAS052"]="cas052" ["CAS061"]="cas061" ["CAS062"]="cas062" ["CAS071"]="cas071" ["CAS072"]="cas072" ["CAS073"]="cas073" ["CN131"]="cn131" ["CN132"]="cn132" ["CN133"]="cn133" ["CN134"]="cn134" ["CN141"]="cn141" ["CN142"]="cn142" ["CN143"]="cn143" ["CN144"]="cn144" ["CN161"]="cn161" ["CN162"]="cn162" ["CN163"]="cn163" ["CN164"]="cn164" ["CN171"]="cn171" ["CN172"]="cn172" ["CN173"]="cn173" ["CN174"]="cn174" ["CN181"]="cn181" ["CN182"]="cn182" ["CN183"]="cn183" ["CN184"]="cn184" ["CN191"]="cn191" ["CN192"]="cn192" ["CN211"]="cn211" ["CN212"]="cn212" ["CN231"]="cn231" ["CN232"]="cn232")
gettruecrbthostname()
{
...
crbtorifilenames=("AS201" "AS202" "AS221" "CAS031" "CAS032" "CAS033" "CAS071" "CAS072" "CAS073" "CAS041" "CAS042" "CAS051" "CAS052" "CAS061" "CAS062" "CN161" "CN162" "CN163" "CN164" "CN171" "CN172" "CN173" "CN174" "CN181" "CN182" "CN183" "CN184" "CN191" "CN192" "CN211" "CN212" "CN231" "CN232")
#音频文件存放目录
crbtfiledir=/home/icaa/loudan/scziyuan/crbt
#音频原始文件存放目录
crbtfileoridir=/home/icaa/loudan/scziyuan/crbtyuanshi
#音频文件临时工作目录
...
#视频原始文件主机名，同音频
declare -A truevrbthostname=(["CAS033"]="cas033" ["CAS032"]="cas032" ["CAS031"]="cas031" ["CAVTAS01"]="cavtas01" ["CAS073"]="cas073" ["CAS072"]="cas072" ["CAS071"]="cas071")
vrbtorifilenames=("CAS033" "CAVTAS01" "CAS073" "CAS032" "CAS031" "CAS072" "CAS071")
#视频文件存放目录
vrbtfiledir=/webjoin/video/logfile/bak/${d1dayago}
#视频原始文件存放目录
vrbtfileoridir=/webjoin/video/logfile/bak/${d1dayago}
#视频文件临时工作目录
...
#		fi
	...
	do
		...
		then
			...
			echo "set lock mode to wait 5;insert into inms_pm_data (jobid,host,account,endtime,seqno,inserttime,statnum,stat01) values ('$crbtorijobid','${truecrbthostnamefanyi}','$wai','$dSn',1,'$dSn',1,'$hostnum')"|dbaccess ${databasename}@${servername}
			if [ $? = 0 ]
			then
			...
			echo "set lock mode to wait 5;insert into inms_pm_datamgr (jobid,host,account,endtime,datanum) values ('$crbtorijobid','${truecrbthostnamefanyi}','$wai','$dSn',1)"|dbaccess ${databasename}@${servername}
			if [ $? = 0 ]
			then
...
#		fi
	...
	do
		...
		then
			...
			echo "set lock mode to wait 5;insert into inms_pm_data (jobid,host,account,endtime,seqno,inserttime,statnum,stat01) values ('$vrbtorijobid','${truevrbthostname[$i]}','$wai','$dSn',1,'$dSn',1,'$hostnum')"|dbaccess ${databasename}@${servername}
			if [ $? = 0 ]
			then
			...
			echo "set lock mode to wait 5;insert into inms_pm_datamgr (jobid,host,account,endtime,datanum) values ('$vrbtorijobid','${truevrbthostname[$i]}','$wai','$dSn',1)"|dbaccess ${databasename}@${servername}
			if [ $? = 0 ]
			then
...
new0errgo()
{
	...
	then
		...
		new0errord2dnum=`grep $d2dayago $jilu|grep $new0errorflag|awk -F'|' '{print $4}'`
...
crbtgo()
{
	...
	if [ $? = 0 ]
	then
		echo "set lock mode to wait 5;insert into inms_pm_data (jobid,host,account,endtime,seqno,inserttime,statnum,stat01,stat02) values ('$alljobid','$hostname','$wai','$dSn',1,'$dSn',2,'${crbt_record_num}','$crbtrecordnum')"|dbaccess ${databasename}@${servername}
		if [ $? = 0 ]
		then
		...
		echo "set lock mode to wait 5;insert into inms_pm_datamgr (jobid,host,account,endtime,datanum) values ('$alljobid','$hostname','$wai','$dSn',1)"|dbaccess ${databasename}@${servername}
		if [ $? = 0 ]
		then
...
vrbtgo()
{
	...
	if [ $? = 0 ]
	then
		echo "set lock mode to wait 5;insert into inms_pm_data (jobid,host,account,endtime,seqno,inserttime,statnum,stat01,stat02) values ('$alljobid','$hostname','$wai','$dSn',1,'$dSn',2,'${vrbt_record_num}','$vrbtrecordnum')"|dbaccess ${databasename}@${servername}
		if [ $? = 0 ]
		then
...
shibai()
{
	...
	then
		...
		echo "set lock mode to wait 5;load from $dodir/failresent_data.unl insert into inms_pm_data"|dbaccess ${databasename}@${servername}
		if [ $? = 0 ]
		then
	...
	then
		...
		echo "set lock mode to wait 5;load from $dodir/failresent_datamgr.unl insert into inms_pm_datamgr"|dbaccess ${databasename}@${servername}
		if [ $? = 0 ]
		then
